...
mongoose.connect('mongodb://127.0.0.1:27017/', { 
    ...
    app.post('/register', async (req, res) => {
        ...
        try {
            if (usertype === 'customer') {
                ...
                const userCreated = await newUser.save();
                return res.status(201).json(userCreated); // Changed from user to userCreated
            }
        } catch (error) {
            console.log(error);
    ...
    app.put('/approve-loan', async (req, res) => {
        ...
        try {
            ...
            const transaction = new Transactions({
                receiverId: user._id,
                receiverName: user.username, // Changed from user.name to user.username
                loan: loan.loanType,
                amount: loan.loanAmount, // Added amount from loan.loanAmount
                time: new Date(),
            ...
            console.error('Error approving loan:', err); // Added error logging
    ...
    app.put('/decline-loan', async (req, res) => {
        try {
            ...
            console.error('Error declining loan:', err); // Added error logging
            res.status(500).json({ message: 'error occurred' });
        }
    });
    ...
    app.post('/repay-loan', async (req, res) => {
        ...
        try {
            ...
            const transaction = new Transactions({
                senderId: user._id,
                senderName: user.username, // Changed from user.name to user.username
                loan: loan.loanType,
                amount,
                time: new Date(),
            ...
            console.error('Error repaying loan:', err); // Added error logging
            res.status(500).json({ message: 'Error occurred during repayment' });
        }
    });
